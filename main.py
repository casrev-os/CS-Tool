...
def check_and_update(local_path, github_raw_url):
    try:
        # GitHub'daki raw içeriğini çek
        response = requests.get(github_raw_url)
        if response.status_code != 200:
            print("❎️ GitHub'dan raw içeriği alınamadı. Status:", response.status_code)
            return
        ...
        if local_code != remote_code:
            with open(local_path, "w", encoding="utf-8") as f:
                f.write(remote_code)
            print("✅️ Kod güncellendi Programı yeniden başlat...", local_path)
            exit()
        else:
            pass
    except:
...
check_and_update(file_path,"https://raw.githubusercontent.com/casrev-os/CS-Tool/refs/heads/main/main.py")
time.sleep(3)
os.system("cls||clear")
